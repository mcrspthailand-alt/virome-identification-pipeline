...
###############################################################################
# Air non-hybrid cohort: rerun DIAMOND with full alignment fields, then merge
# readcounts/length + evidence + CheckV + optional taxonomy/lineage in ONE script.
#
...
def count_prodigal_orfs(samples_dir):
    rows = []
    for faa in glob.glob(os.path.join(samples_dir, "*", "07_diamond_option1_all_depth_ge2_*", "prodigal_out", "*.proteins.faa"), recursive=True):
