...
###############################################################################
# Unified sediment virome pipeline
...
ASSEMBLY_DIR="${PROJECT_ROOT}/results/Assembly/MEGAHIT"
READS_DIR="${PROJECT_ROOT}/results/QC_shortreads/remove_host"
...
# TOOL CHECKS
# =========================
for exe in bash awk zcat samtools minimap2 mosdepth prodigal diamond seqkit bbduk.sh python3; do
...
[[ -d "$ASSEMBLY_DIR" ]] || { echo "[ERROR] assembly dir not found: $ASSEMBLY_DIR"; exit 1; }
[[ -d "$READS_DIR" ]] || { echo "[ERROR] reads dir not found: $READS_DIR"; exit 1; }
...
MASTER_LOG="${LOG_DIR}/sed_full.master.log"
STATUS_TSV="${LOG_DIR}/sed_full.status.tsv"
echo -e "sample\tstatus\tmessage" > "$STATUS_TSV"
echo "=== sediment full batch started: $(date) ===" | tee "$MASTER_LOG"
...
run_one_sample() {
  ...
  local ASM_GZ="${ASSEMBLY_DIR}/MEGAHIT-${SAMPLE}.contigs.fa.gz"
  [[ -s "$ASM_GZ" ]] || { echo -e "${SAMPLE}\tFAILED\tmissing assembly: ${ASM_GZ}" >> "$STATUS_TSV"; return 1; }

  local pair
  if ! pair="$(find_reads_for_sample "$SAMPLE")"; then
    echo -e "${SAMPLE}\tFAILED\tmissing paired FASTQ in ${READS_DIR}" >> "$STATUS_TSV"
    return 1
  fi
  local R1="${pair%%|*}"
  local R2="${pair##*|}"
...

  {
    ...
    mkdir -p "$D00" "$D01" "$D02" "$D03" "$D04" "$D05" "$D07_500" "$D07_1000" "$D08"

    ln -sfn "$R1" "${D00}/${SAMPLE}_R1.fastq.gz"
    ln -sfn "$R2" "${D00}/${SAMPLE}_R2.fastq.gz"
    ln -sfn "$ASM_GZ" "${D00}/${SAMPLE}.assembly.contigs.fa.gz"
...
# Merge stage
###############################################################################
READCOUNT_MERGED="${MERGED_DIR}/ALL_samples.readcounts.by_lenbin.tsv"
...
MASTER_COMBINED="${MERGED_DIR}/ALL_samples.master_option1.combined.tsv"
...
MASTER_WITH_MAPPED="${MERGED_DIR}/ALL_samples.master_option1.combined.fixed.with_mapped_reads.tsv"
...
TIER1_READS_MATRIX="${MERGED_DIR}/tier1_reads_matrix.combined.tsv"
...
out_reads = os.path.join(outdir, 'tier1_class_reads_matrix.tsv')
out_pa = os.path.join(outdir, 'tier1_class_presence_absence.tsv')
out_long = os.path.join(outdir, 'tier1_class_by_sample.tsv')
...
echo "=== sediment full batch finished: $(date) ===" | tee -a "$MASTER_LOG"
