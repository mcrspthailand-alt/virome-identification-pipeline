...
###############################################################################
# Stage 1 engine: process air samples for viral evidence analysis.
#
# IMPORTANT
...
WITH_TAXID=""   # e.g. /path/to/virus_names_with_taxid.tsv
...
run_one_sample() {
  ...
  if [[ -s "${SAMPLE_OUT}/07_diamond_ge1000/diamond_out/contig_best_hit.filtered.tsv" ]] \
     && [[ -s "${SAMPLE_OUT}/07_diamond_len500_999/diamond_out/contig_best_hit.filtered.tsv" ]] \
...

  {
    ...
    local D07_500="${SAMPLE_OUT}/07_diamond_len500_999"
    local D07_1000="${SAMPLE_OUT}/07_diamond_ge1000"
    ...
    run_option1_diamond() {
      ...
      local D07="${SAMPLE_OUT}/07_diamond_${LEN_BIN}"
...
for sample_dir in sorted(glob.glob(os.path.join(samples_dir, '*'))):
    ...
    for len_bin, p in [
        ('ge1000', os.path.join(sample_dir, '07_diamond_ge1000', 'diamond_out', 'contig_best_hit.filtered.tsv')),
        ('len500_999', os.path.join(sample_dir, '07_diamond_len500_999', 'diamond_out', 'contig_best_hit.filtered.tsv')),
    ]:
        if os.path.exists(p):
...
VIRUS_READS_MATRIX_INITIAL="${MERGED_DIR}/virus_reads_matrix_initial.tsv"
python3 - <<'PY' "$MASTER_WITH_MAPPED" "$VIRUS_READS_MATRIX_INITIAL"
import sys, pandas as pd
...
PY
  ...
  python3 - <<'PY' "$VIRUS_READS_MATRIX_INITIAL" "$MAP_TSV" "$MERGED_DIR"
