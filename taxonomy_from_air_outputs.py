...
DEFAULT_PROJECT_ROOT = "/home/panda/workspace/projects/nextflow_metagenomic/eDNA/air"
DEFAULT_OUT_BASE = os.path.join(DEFAULT_PROJECT_ROOT, "non_hybrid", "viral_id_pipeline")
DEFAULT_MERGED_DIR = os.path.join(DEFAULT_OUT_BASE, "merged")
DEFAULT_MASTER = os.path.join(DEFAULT_MERGED_DIR, "all_viral_evidence.tsv")
DEFAULT_READS_MATRIX = os.path.join(DEFAULT_MERGED_DIR, "virus_reads_matrix_initial.tsv")
...
def write_name_parsing_outputs(master_df: pd.DataFrame, tax_dir: str):
    ...
    with open(os.path.join(tax_dir, "virus_names_cleaned.txt"), "w", encoding="utf-8") as fh:
...
def enrich_from_existing_outputs(args):
    ...
    parsed_master = os.path.join(out_dir, "all_viral_evidence.parsed_organisms.tsv")
    ...
    with_taxid_path = os.path.join(out_dir, "virus_names_with_taxid.tsv")
    ...
    rebuilt_reads = os.path.join(out_dir, "virus_reads_matrix.tsv")
    ...
    out_reads = os.path.join(out_dir, "virus_reads_matrix_with_taxonomy.tsv")
    out_pa = os.path.join(out_dir, "virus_presence_absence_matrix.tsv")
    out_long = os.path.join(out_dir, "virus_reads_by_sample_long.tsv")
...
def main():
    ap = argparse.ArgumentParser(description="Post-process existing v2 air pipeline outputs and assign taxonomy from NCBI taxdump.")
